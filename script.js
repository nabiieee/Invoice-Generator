...
function generateInvoiceNumber() {
  const invoiceField = document.getElementById("invoiceNumber");
  if (invoiceField) {
    let count = parseInt(localStorage.getItem("invoiceCounter")) || 1;
    const now = new Date();
    const dateCode = `${now.getFullYear()}${(now.getMonth() + 1)
      .toString()
      .padStart(2, "0")}`;
    invoiceField.value = `INV-${dateCode}-${count}`;
    localStorage.setItem("invoiceCounter", count + 1);
  }
}
